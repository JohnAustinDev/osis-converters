...
def convertToOsis(sFile,genBook,encoding,relaxedConformance):
    ...
    def cvtFootnotes(osis, relaxedConformance):
        ...
        # \f_+_...\f*
        osis = re.sub(r'\\f\s+([^\s\\]+)?\s*(.+?)\s*\\f\*', lambda m: '<note' + ((' n=""') if (m.group(1) == None or m.group(1) == '-') else ('' if (m.group(1) == '+') else (' n="' + m.group(1) + '"'))) + ' placement="foot">' + m.group(2) + '\uFDDF</note>', osis, flags=re.DOTALL)
...
def readIdentifiersFromOsis(filename):
    ...
    if osisBook and osisBook.group(1) in bookDict:
        osisBook = bookDict[osisBook.group(1)]
        filename2osis[filename] = osisBook
        
    elif osisBook:
        osisBook = 'Gen'
        bookDict[osisWork] = osisBook
        filename2osis[filename] = osisBook
