...
# usage: osis2osis [Bible_Directory]
...
if (! -e $commandFile) {&Error("Cannot run osis2osis without a CF_osis2osis.txt command file located at: $MAININPD.", '', 1);}
...
if (&runCF_osis2osis('postinit')) {
  ...
  &Warn("The osis2osis script did not produce $MOD.", 
