...
<div type="introduction" resp="'.$ROC.'">
  ...
  <title level="1" type="main" subType="x-introduction" canonical="false">'.$translationTitle.'</title>
    ...
    foreach my $bookGroup (@bookGroups) {
      ...
      my $singleNonBookChild = (@{$XPC->findnodes('child::osis:div[not(@type="book")]', $bookGroup)} == 1 ? 'true()':'false()');
      my $bookGroupIntroTOCM = @{$XPC->findnodes('child::*[1][self::osis:div[not(@type="book")]]
      [boolean(following-sibling::*[1][self::osis:div[not(@type="book")]]) or '.$singleNonBookChild.']
