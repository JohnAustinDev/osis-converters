...
sub customize_conf($$$$) {
  ...
  my $confP = &readConf($conf);
  if ($confP->{'REPOSITORY'}) {
    my $cfile = $confP->{'REPOSITORY'}.'/'.lc($modName).".conf";
    my $ctext = &shell("wget \"$cfile\" -q -O -", 3);
    $ctext =~ s/\n#Autogenerated by osis-converters:.*$//s;
    &setConfFileValue($conf, 'REPOSITORY', '', 1);
    if ($ctext) {
      &Note("Default conf was located in REPOSITORY: $cfile\n$ctext\n");
