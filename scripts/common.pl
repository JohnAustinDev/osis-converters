...
sub getSwordConfFromOSIS($) {
  ...
  if ($swordConf{"SourceType"} eq 'OSIS') {
    ...
    # check scope
    if (($type eq 'bible' || $type eq 'commentary') && &getScope($moduleSource) ne $swordConf{'Scope'}) {
      &ErrorBug("Scope of $moduleSource is different than swordConf: ".&getScope($moduleSource)." != ".$swordConf{'Scope'});
    }
  }
