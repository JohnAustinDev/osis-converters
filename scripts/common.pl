...
# Copy conf starter to $conf, check relevent entries, and add necessary entries 
sub updatedSwordConf($) {
  my $conf = shift;

  copy($CONFFILE, $conf);
  &normalizeNewLines($conf);
  
  my $sourceType = ($IS_usfm2osis && $MODDRV =~ /LD/ ? 'TEI':'OSIS');
  my $type = '';
  my $infile = $OSISFILE;
  if ($MODDRV =~ /LD/) {$type = 'dictionary'; if (!$IS_usfm2osis) {$infile = $IMPFILE;}}
  ...
  print CONF "\n\n#Autogenerated by osis-converters:\nDataPath=". $ConfEntry{"DataPath"} . "\n";
  if ($sourceType eq "OSIS") {
    if ($ConfEntry{"GlobalOptionFilter"} !~ /OSISFootnotes/) {print CONF "GlobalOptionFilter=OSISFootnotes\n";}
    if ($ConfEntry{"GlobalOptionFilter"} !~ /OSISHeadings/)  {print CONF "GlobalOptionFilter=OSISHeadings\n";}
    if ($ConfEntry{"GlobalOptionFilter"} !~ /OSISScripref/)  {print CONF "GlobalOptionFilter=OSISScripref\n";}
  }

  if ($ConfEntry{"Encoding"} && $ConfEntry{"Encoding"}  ne "UTF-8") {
    &Log("ERROR: Encoding is set incorrectly in $CONFFILE. Remove this entry.\n");
  }
  if ($ConfEntry{"SourceType"} && $ConfEntry{"SourceType"}  ne $sourceType) {
  ...
  if ($ConfEntry{"Encoding"}  ne "UTF-8") {
    print CONF "Encoding=UTF-8\n";
  }
  if ($ConfEntry{"SourceType"}  ne $sourceType) {
    print CONF "SourceType=$sourceType\n";
  }
  if ($ConfEntry{"OSISVersion"}  ne $OSISVersion) {
    print CONF "OSISVersion=$OSISVersion\n";
  }
