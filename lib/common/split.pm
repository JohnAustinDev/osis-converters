...
# Duplicate an OSIS file as a series of files: one file having all $xpath
# elements removed (called 'other') and then files having exactly one
# $xpath target element, with all other targets removed. This method provides
# a massive speedup compared to searching or modifying one huge nodeset. It
# is intended for use with joinOSIS which will reassemble these OSIS files
# back into one document, after they are searched and/or processed.
# IMPORTANT: external processing may ONLY modify the specific target
# element within the OSIS file, except for the 'other' file where ONLY
# non-target nodes may be modified. The splitOSIS_element()returns target
# elements of the file for that purpose.
sub splitOSIS {
  my $in_osis = shift;
  my $xpath = shift;
...
# Take a split file created by splitOSIS and return its target element. If
# $xmlP pointer is passed, it will point to the file's parsed document node.
# If $filterP pointer is passed, it will be written with an xpath predicate
# that may be used to select target nodes within the split file (useful
# when the exact number of changes made is required for instance). If $xpath
# was passed to splitOSIS, the same value must be passed here.
sub splitOSIS_element {
  ...
  my @e = @{$XPC->findnodes("//${xpath}[\@osisID='$osisID']", $xml)};
