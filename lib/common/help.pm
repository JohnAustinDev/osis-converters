...
'sfm2osis' => [
  ...
  ['SYNOPSIS', [
    ...
    ['para', 'A project conversion creates a main OSIS file, which may contain a Bible, Children\'s Bible or a commentary. Paratext glossaries, maps and other reference materials, if they exist, are converted into a second OSIS file. These are converted as separate modules, referred to as MAINMOD and DICTMOD. If a project has a DICTMOD, it appears as a subdirectory within the main module. All DICTMOD module names end with `DICT`.' ],
  ...
  ['CF_sfm2osis.txt', [
    ...
    ['para', 'The other purpose of the `CF_sfm2osis.txt` file for Bibles and commentaries is to describe deviations from the versification standard. These deviations should be recorded so references from external documents may be properly resolved, and parallel rendering with other texts can be accomplished. Each verse is identified according to the project\'s strictly defined versification. The commands to accomplish this begin with VSYS. Their proper use results in OSIS files containing both a rendering in the translator\'s custom versification and a rendering in the standard versification. OSIS files can then be rendered in either scheme using an XSLT stylesheet. ' ],
