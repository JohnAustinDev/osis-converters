...
'sfm2osis' => [
  ...
  ['CF_sfm2osis.txt', [
    ...
    ['para', 'The other purpose of the `CF_sfm2osis.txt` file for Bibles and commentaries is to describe deviations from the versification standard. These deviations should be recorded so references from external documents may be properly resolved, and parallel rendering with other texts can be accomplished. Each verse is identified according to the project\'s strictly defined versification. The commands to accomplish this begin with VSYS. Their proper use results in OSIS files containing both a rendering of the translator\'s custom versification and a rendering of the standard versification. OSIS files can then be rendered in either scheme using the corresponding XSLT stylesheet. ' ],
  ...
  ['CF_addDictLinks.xml', [
    ...
    ['para', 'Glossary references marked by translators are called explicit references. If the target of an explicit reference cannot be determined, a conversion error is logged. USFM 3 explicit references may include their target lemma in the `lemma` attribute value. If there are multiple sub-publications which share the same lemma in their glossaries, disambiguation may still be required, however. Two additional `\w` tag attributes may be used for disambiguation: `context` to specify the context of the target glossary (usually a scope value), and `dup` to specify the number of the duplicate lemma as it appears in CF_addDictLinks.xml.'],
