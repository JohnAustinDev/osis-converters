...
'convert' => [
  ['SYNOPSIS', [
    ...
    ['sub-heading', 'SCOPE' ],
    ['para', 'A \'scope\' is a specific way of listing the contents of Bible publications. It is generally a space separated list of OSIS book abbreviations in verse system order (see HELPREF(OSIS_ABBR)). When used in directory names, file names or config.conf entry names, spaces should be replaced by underscores. Example: `Ruth_Esth_Jonah`. Continuous ranges of more than two books are shortened using \'-\'. Example: `Matt-Rev`.' ],
...
'sfm2osis' => [
  ...
  ['SYNOPSIS', [
    ...
    ['list', ['DIRECTIVE', 'DESCRIPTION'], &addDirectiveType(&getList( [map(@{$ID_DIRECTIVES{$_}}, reverse sort keys %ID_DIRECTIVES)], [
      ...
      ['subType', 'Marks OSIS div elements with a subType name attribute. Mainly this attribute just facilitates grouping of entries in the default CF_addDictLinks.xml files that are created during the sfm2osis conversion. The subType value must match `x-([^\s])+` for the OSIS to validate.' ],
    ...
    ['list', ['FEATURE', 'DESCRIPTION'], [
      ...
      \b`RUN:../sfm/FRT.SFM`' ],
      ['NAVMENU', 'Navigation menus are created automatically from the TOC. But when custom navigation menus are desired, the NAVMENU feature may be used. Design the custom menu in USFM and append the ID directive to its `\id` tag:
    ...
    ['para', 'Scripts with these names in a module directory will be called at different points during the conversion to OSIS:' ],
    ['list', ['HOOK', 'WHEN CALLED'], [
      ['bootstrap.pl', 'The sfm2osis or osis2osis script will execute it before conversion begins. It must appear in a project\'s top directory, and it takes no arguments. It may be used to copy or process any project file etc., but should be used only when osis2osis is insufficient (see HELPREF(osis2osis)).' ],
  ...
  ['config.conf', [
    ...
    ['list', ['ENTRY', 'DESCRIPTION'], &addConfigType(&getList(&configList(), [
      ...
      [ 'TitleCase' => 'A number from 0 to 3 selecting letter casing for table of contents titles: 0 is as-is, 1 is Like This, 2 is LIKE THIS, 3 is Like this.' ],
      [ 'TitleTOC' => 'A number from 1 to 3 indicating which Bible book title to use for the TOC and single book publications: 1 uses the BookNames.xml `long` name or the book `\toc1` tag, 2 uses the BookNames.xml `short` name or the book `\toc2` tag, and 3 uses the BookNames.xml `abbr` name or book `\toc3` tag.' ],
  ...
  ['CF_sfm2osis.txt', [
    ...
    ['para', 'The other purpose of the `CF_sfm2osis.txt` file for Bibles and commentaries is to describe deviations from the versification standard. These deviations should be recorded so references from external documents may be properly resolved, and parallel rendering with other texts can be done. Each verse is identified according to the project\'s strictly defined versification. The commands to accomplish this begin with VSYS. Their proper use results in OSIS files containing both a rendering of the translator\'s custom versification and a rendering of the standard versification. OSIS files can then be rendered in either scheme using the corresponding XSLT stylesheet. ' ],
    ['para', 'VSYS instructions are evaluated in verse system order regardless of their order in the control file. A verse may be affected by multiple VSYS instructions. VSYS operations on entire chapters are not supported except for VSYS_EXTRA chapters at the end of a book (such as Psalm 151 of Synodal).'],
  ...
  ['CF_vsys.xml', [
    ...
    ['para', 'A strictly defined address is assigned to each verse, making it is possible to incorporate a list of cross-references into the translation. These cross-references, although not part of the original translation, can be an excellent Bible study tool. The cross-reference list must belong to the same versification system as the project. The list must be placed in PATH(AddCrossRefs/CF_<vsys>.xml) within a `defaults` directory (see HELPREF(defaults)). `<vsys>` is the project\'s versification system (options are: '.join(', ', map("`$_`", @VERSE_SYSTEMS)).'). These verse systems are defined in `canon_<vsys>.h` of [SWORD](https://crosswire.org/svn/sword/trunk/include/). Verse maps between these verse systems are defined in `<vsys>.properties` of [JSWORD](https://github.com/crosswire/jsword/tree/master/src/main/resources/org/crosswire/jsword/versification)'],
    ...
    ['para', 'The `CF_<vsys>.xml` file is an OSIS file with books, chapters and verses following the versification system; the only content required however are OSIS notes. Example OSIS notes:
  ...
  ['CF_addDictLinks.xml', [
    ...
    ['para', 'Glossary references that are marked by translators are called explicit references. If the target of an explicit reference cannot be determined, a conversion error is logged. USFM 3 explicit references may include their target lemma in the `lemma` attribute value. If there are multiple sub-publications which share the same lemma in their glossaries, disambiguation may still be required, however. Two additional `\w` tag attributes may be used for disambiguation: `x-context` to specify the context of the target glossary (usually a scope value), and `x-dup` to specify the number of the duplicate lemma as it appears in CF_addDictLinks.xml.'],
    ['para', 'Glossary references that are not marked explicitly by the translators may be parsed from the text using the CF_addDictLinks.xml file. But a classic problem when parsing glossary links is determining the actual meaning of each parsed link in context. For example, a language may use the same word to mean both \'sky\' and \'heaven\', and if the glossary entry is about heaven, then links with the \'sky\' meaning should not be made. Thankfully, AI is perfecty suited to accomplish this difficult task. To use AI, follow these steps:' ],
    ['para', '1) Finish your configuration of CF_addDictLinks.xml and run `sfm2osis` which will add glossary links to the text. The resulting `LOG_sfm2osis.txt` file will be required by the next step.' ],
    ['para', '2) Run `bin/rankGlossaryTerms` which will generate a series of AI prompts in the `ai_prompt_terms` directory. Each of these prompts should be fed to AI (use Google Gemeni which seems to have the best comprehension of smaller languages). The AI should return a comma separated value (csv) text snippet that you will need to append to the `terms.csv` file (the `terms.csv` file will also be found in the `ai_prompt_terms` directory). The purpose of this step is for AI to determine which glossary terms are likely to be have been parsed correctly already, and which will require further scrutiny and filtering.' ],
    ['para', '3) Next run `bin/rankGlossaryLinks` which will generate a new series of AI prompts in the `ai_prompt_links` directory. Just as with step 2, these prompts are to be fed to the AI, but the results should be appended instead to the `links.csv` file in the `ai_prompt_links` directory. The purpose of this step is for AI to determine which individual links should be filtered out by CF_addDictLinks.xml.' ],
    ['para', '4) Finally run `bin/filterAddDictLinks` which will read your completed `terms.csv` and `links.csv` files and write the required `context` and `notContext` attributes to your CF_addDictLinks.xml file. Now, future runs of `sfm2osis` will apply the AI updated CF_addDictLinks.xml, filtering glossary links according to their individual meaning.' ],
    ...
    ['list', ['ATTRIBUTE', 'DESCRIPTION'], &addAttributeType(&getList([ sort keys %{$CF_ADDDICTLINKS{'attributes'}} ], [
      ...
      ['notExplicit',  'If the value is `true` (or else contains a context matching the text node) the `match` element will not be applied to explicitly marked references.' ],
      ['context',    'The `match` element will only be applied to text nodes included in this context value.' ],
      ['notContext', 'The `match` element will not be applied to text nodes included in this context value.' ],
      ['XPATH',    'The `match` element will only be applied to text nodes for which this xpath value evaluates as true.' ],
      ['notXPATH', 'The `match` element will not be applied to text nodes for which this xpath value evaluates as true.' ],
      ['dontLink', 'If `true`, then the `match` element will instead undo reference links, rather than create them.' ],
      ['onlyOldTestament', 'If `true`, the `match` element will only be applied to text nodes in the Old Testament.' ],
      ['onlyNewTestament', 'If `true`, the `match` element will only be applied to text nodes in the New Testament.' ],
    ], 1))],
...
'osis2ebooks' => [
  ...
  ['SYNOPSIS', [
    ...
    ['list', ['ENTRY', 'DESCRIPTION'], [
      ...
      ['MakeSet[comp]', 'HELP(sfm2osis;config.conf;MakeSet[comp])' ],
      ['MakeSet[full]', 'HELP(sfm2osis;config.conf;MakeSet[full])' ],
...
'osis2gobible' => [
  ...
  ['SYNOPSIS', [
    ...
    ['para', 'Default control files will be copied from a `defaults` directory (see HELPREF(defaults)). This includes the Go Bible Creator user interface localization file and the app icon. These files may be customized per project, or customized for a group of projects, depending on which `defaults` directory is used.' ],
