...
sub searchGlossaryLinkAtIndex {
  ...
  my $failReasonP = shift; # optional pointer to explicit link failure
  ...
  my $fail = $failReasonP ? $$failReasonP : '';
  if (!$expcon && $infoP->{'linktext'}) {
    ...
    $match = &searchText(\$context, $indexElement, $glossaryHP, length($context), \$fail);
  }
  else {
    ...
    $match = &searchText(\$context, $indexElement, $glossaryHP, $index, \$fail);
  }

  if (!$match) {
    # If unidirectional search failed, try bi-directional
    if (!$infoP->{'linktext'} && !$bidir) {
      my $iel = searchGlossaryLinkAtIndex($indexElement, $glossaryHP, 1, \$fail);
      if (0 + $iel == 0 + $indexElement && $fail) {
        &Error("Explicit link (bi-directional context) could not be matched because of: '$fail'");
      }
      return $iel;
    }
...
sub searchText {
  ...
  my $failReasonP = shift; # Reason for explicit match failure
