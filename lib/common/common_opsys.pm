...
our @CONFIG_DEPRECATED = ('LangSortOrder', 'SPECIAL_CAPITALS', 
  'PUNC_AS_LETTER', 'SET_MODE_CCTable');
...
our @SWORD_OC_CONFIGS = ('Scope', 'KeySort', 'AudioCode', 'LangSortOrder');

# These are entries which should NOT appear in config.conf, because 
# they are autogenerated only for SWORD config.conf files
our @SWORD_AUTOGEN_CONFIGS = (
  'DataPath', 'Category', 'CompressType', 'BlockType', 
...
our @OC_CONFIGS = (
  ...
  'CustomBookOrder', 'IntroductionTitle', 'ProjectType',
);
...
our @OC_SYSTEM_CONFIGS = (
  ...
  'VAGRANT', 'NO_FORKS', 'RAM_GB_EBOOKS', 'RAM_MB_EBOOKS_PERBOOK'
);
our $RAM_GB_EBOOKS_DEF = 1;
our $RAM_MB_EBOOKS_PERBOOK_DEF = 32;
...
our @OC_LOCALIZABLE_CONFIGS = (
  'MATCHES:SubPublicationTitle\[(?<scope>\S+)\]$', 'MATCHES:ARG_\w+Title\d', 
...
# Default values for config entries which have a default value. Values
# ending in 'DEF' will trigger an error if used, to let the developer
# know a value is required.
our %CONFIG_DEFAULTS = (
  'ProjectType' => 'bible',
  ...
  'BookGroupTitle[OT]' => 'Old Testament DEF',
  'BookGroupTitle[NT]' => 'New Testament DEF',
...
our %SWORD_CONFIG_VALUES = (
  'all' => {
    'SourceType' => 'OSIS',
    'OSISVersion' => '$osisversion',
    'Encoding' => 'UTF-8', 
    'GlobalOptionFilter' => join('<nx/>', 'OSISFootnotes', 'OSISHeadings', 'OSISScripref'),
    'SwordVersionDate' => '$today',
    'PreferredCSSXHTML' => '$modulecss',
  ...
  'bible' => {
    ...
    'ModDrv' => 'zText', 
    'DataPath' => './modules/texts/ztext/$mod/', 
  ...
  'childrens_bible' => {
    'Companion' => '$chblCompanion',
    'ModDrv', => 'RawGenBook',
    'DataPath' => './modules/genbook/rawgenbook/$mod/$mod',
  },
  'commentary' => {
    'ModDrv', => 'RawCom4',
    'DataPath' => './modules/comments/rawcom4/$mod/',
    'Scope' => '$scope',
  },
  'dictionary' => {
    'Companion' => '$MAINMOD',
...
sub set_project_globals {
  ...
  { my $subs = join('|', 'sfm', 'images', 'output', &PUB_TYPES());
    $INPD =~ s/\/($subs)(\/.*?$|$)//;
  }
...
"Project directory \"$INPD\" does not exist. Check your command line.", 1);
  ...
  # after defaults() in case a new dictionary is discovered in the USFM).
  our $MOD = $INPD; $MOD =~ s/^.*\///;
  ...
  our $CONF; our $CONFFILE = "$MAININPD/config.conf";
  &readSetCONF();
  ...
  { my $dictncd = join('|', @{$CONV_NOCANDO{'dictionary'}});
    if ($INPD eq $DICTINPD && 
...
# Read the configuration files for a project. Returns a hash pointer
...
# overwritten if previously set. The keywords MAINMOD or DICTMOD will
# cause the value of $MAINDMOD to be written. Otherwise, the first 
# unknown section name not ending in DICT will become the MAINMOD key 
# value, and once written it will not be changed.
# 
# VALUES:
    ...
    # handle section headings
      ...
      if ($section =~ /$sectRE/) {
        if ($section eq 'MAINMOD') {
          if (!$confP->{'MAINMOD'}) {$confP->{'MAINMOD'} = $MAINMOD;}
        ...
        if (!$confP->{'MAINMOD'}) {&ErrorBug("MAINMOD is not set.", 1);}
      }
      elsif (!$confP->{'MAINMOD'}) {
        $confP->{'MAINMOD'} = ($section =~ /^(\w+)DICT$/ ? $1 : $section);
      }
    }
...
# is set, and a [system] section will be included, signaling an 
# osis-converters 1.0+ config file.
sub writeConf {
  my $file = shift;
  my $confP = shift;
  ...
  if (open(XCONF, $WRITELAYER, $file)) {
    ...
    if ($filterDefaults && !$hasSystemSection) {
      print XCONF "\n[system]\n#DEBUG=1\n";
    }
    close(XCONF);
...
# undef). If priority 1, 2 or 3 is specified, only the location with 
# that priority will be checked:
#
# priority  location(s)
...
#    2      $projdir/(../)*defaults directory
#    3      osis-converters/defaults directory
#
# $projdir may be either $MAININPD or $MAININPD/../
sub getDefaultFile {
  my $file = shift;
  my $priority = shift;
  ...
  # For project dir, replace DICTMOD with the actual module name
  my $mod = $projdir; $mod =~ s/^.*?([^\/]+)\/?$/$1/;
  my $lfile = $file; $lfile =~ s/^DICTMOD\//${mod}DICT\//;
  my $ret = "$projdir/$lfile";
...
END {
  ...
  if ($p) {print "Dumping log buffer. LOGFILE=$LOGFILE\n";}
}
