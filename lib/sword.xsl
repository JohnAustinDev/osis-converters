...
<stylesheet version="2.0"
 ...
 xmlns:me="http://github.com/JohnAustinDev/osis-converters/sword.xsl"
...
<with-param name="exp">Some SWORD front-ends like xulsword cannot handle multi-chapter reference ranges.</with-param>
  ...
  <!-- Prefix 2 dashes to $uiIntroduction keyword, and one dash to
  $uiDictionary keyword, and update all osisRefs argetting them. This will put
  these two keywords at the top of the SWORD DICT module. -->
  <template mode="dashPrefix" match="seg[@type='keyword']">
        ...
        <when test="self::seg[normalize-space(text()) and text() = $reftext]">
          <value-of select="
              concat(
                if (
                    text() =
                    oc:decodeOsisRef(tokenize($REF_introductionINT, ':')[2])
                  )
                  then '--'
                  else '-',
                ' ',
    ...
    <!-- Implement a harmless work around for xulsword which doesn't handle ':'
    in keyword values, by replacing ':' in non TOC osisRef values with '-'.  -->
...
<with-param name="msg">The xulsword front-end does not allow ':' in keywords. Changing '<value-of select="$text1"/>' to '<value-of select="$text"/>'</with-param>
  ...
  <!-- SWORD DICT references can only target keyword IDs, but DICT_TOP is a
  milestone, so redirect references to DICT_TOP to the top NAVMENU keyword. -->
  <template mode="dashPrefix" match="@osisRef">
    <choose>
      <when test=". = concat($DICTMOD, ':DICT_TOP')">
        <attribute name="osisRef" select="
            me:dashPrefix(
              if ($INT_feature) then $REF_introductionINT else $REF_dictionary
            )"/>
      </when>
      <otherwise>
        <attribute name="osisRef" select="me:dashPrefix(.)"/>
      </otherwise>
    </choose>
  </template>

  <function name="me:dashPrefix" as="xs:string">
    <param name="osisRef" as="xs:string"/>

    <variable name="osisRef1" select="
        if (oc:ref($osisRef) and $osisRef = ($REF_introductionINT, $REF_dictionary))
        then concat(
          $DICTMOD, ':',
          if ($osisRef = $REF_introductionINT) then '_45__45__32_' else '_45__32_',
          tokenize($osisRef, ':')[2])
        else $osisRef"/>
    <!-- Implement a harmless work around for xulsword which doesn't handle ':'
    in keyword values, by replacing ':' in non TOC osisRef values with '-'.  -->
    <variable name="osisRef2" select="
        concat(
          tokenize($osisRef1, ':')[1],
          ':',
          oc:encodeOsisRef(
              replace(oc:decodeOsisRef(tokenize($osisRef1, ':')[2]), ':', '-')
            )
        )"/>
    <value-of select="
        if (
          tokenize($osisRef1, ':')[1] = $DICTMOD and
          not(contains($osisRef1, '!'))
        )
        then $osisRef2
        else $osisRef1"/>
  </function>
