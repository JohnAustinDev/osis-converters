...
<stylesheet version="2.0"
 ...
 exclude-result-prefixes="#all">
  ...
  <template match="/">
    <variable name="fitted"><call-template name="fittedVerseSystem.xsl"/></variable>
    <variable name="whitespace"><apply-templates mode="whitespace.xsl" select="$fitted"/></variable>
    <sequence select="$whitespace"/>
  </template>
  
  <template mode="fittedVerseSystem.xsl" match="/" name="fittedVerseSystem.xsl">
    <message>NOTE: Running fittedVerseSystem.xsl</message>
