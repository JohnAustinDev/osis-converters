...
<stylesheet version="2.0"
 ...
 exclude-result-prefixes="#all">
  ...
  <!-- needed for reporting results and removedKeywords -->
  <import href="./common/functions.xsl"/>

  <param    name="conversion" as="xs:string"/><!-- means 'only conversion' -->
  ...
  <template mode="conversion.xsl" match="/" name="conversion.xsl">
    ...
    <if test="$removeElements">
      <call-template name="Note">
        <with-param name="msg">Removed <value-of select="count($removeElements)"/> elements(s) marked as '<value-of select="string-join(distinct-values($removeElements/tokenize(@annotateRef, '\s+')), ' ')"/>' during conversion to '<value-of select="$conversion"/>'.</with-param>
      </call-template>
    </if>
...
<with-param name="exp">Pass DICTMOD_URI and MAINMOD_DOC to conversion.xsl to enable checking and forwarding of these references.</with-param>
  ...
  <template mode="conversion" match="@osisRef">
    <attribute name="osisRef" select="
        replace(
          oc:filter_osisRef(., true(), $removedOsisIDs),
          '\.dup\d+!toc',
          '!toc'
        )"/>
  </template>
