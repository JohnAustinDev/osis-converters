...
"If you want to include it, add to config.conf the entry: $confentry=<title>");
    ...
    foreach my $bookGroup (@bookGroups) {
      ...
      # bookGroup when it is either the only non-book TOC div or else is 
      # immediately followed by another non-book TOC div.

      # Is there already a bookGroup introduction TOC (in other words 
      # not autogenerated)?
      my $singleNonBookChild = (
        @{$XPC->findnodes('child::osis:div[not(@type="book")]
                          [not(@resp="'.$ROC.'")]', $bookGroup)} == 1 ? 
                          'true()' : 'false()');
      my $bookGroupIntroTOCM = 
        @{$XPC->findnodes('child::*[1]
          [self::osis:div[not(@type="book")]][not(@resp="'.$ROC.'")]
          [ boolean( following-sibling::*[1]
              [self::osis:div[not(@type="book")][not(@resp="'.$ROC.'")]]
            ) or '.$singleNonBookChild.'
          ]
          /child::osis:milestone[@type="x-usfm-toc'.$toc.'"]'
          , $bookGroup)}[0];
...
$bookGroupIntroTOCM->getAttribute('n')."'.");
      ...
      my @bookSubGroupAuto = $XPC->findnodes(
          'child::osis:div[not(@type="book")][not(@resp="'.$ROC.'")][@scope]
          [ preceding-sibling::*[not(@resp="'.$ROC.'")][1]
            [self::osis:div[@type="book"]]
          ] |
          child::*[not(@resp="'.$ROC.'")][position()=1 or position()=2]
          [self::osis:div[not(@type="book")][@scope]]'
      , $bookGroup);
      for (my $x=0; $x<@bookSubGroupAuto; $x++) {
        if (@bookSubGroupAuto[$x] && 
            $bookGroupIntroTOCM && 
            @bookSubGroupAuto[$x]->unique_key eq 
            $bookGroupIntroTOCM->parentNode->unique_key) {
