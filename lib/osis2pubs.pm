...
"convertOSIS: Conversion of OSIS to \"$convertTo\" is not yet supported.");
  ...
  my $fullScope = '';
  if (!&isChildrensBible($INOSIS_XML) && !&isGenericBook($INOSIS_XML)) {
...
file for it, and then run this script again.");}
  ...
  if ($convertTo eq "html") {
    ...
    &Log("--- CHECKING html links in \"$PUBOUT/$pubName/index.html\"\n");
    my $result = &shell("linkchecker \"$PUBOUT/$pubName/index.html\"", 3);
...
"Check the version of linkchecker.");}
    ...
    &Log("\n--- CHECKING for unreachable material in \"$PUBOUT/$pubName/html\"\n");
    my @files = split(/\n+/, &shell("find \"$PUBOUT/$pubName/html\" -type f", 3, 1));
    push(@files, "$PUBOUT/$pubName/index.html");
    my %linkedFiles;
    $linkedFiles{&shortPath("$PUBOUT/$pubName/index.html")}++;
...
of this material to exclude it from HTML publications.");
    ...
    &Report("Found $numUnreachable unreachable file(s) in '$PUBOUT/$pubName/html'");
  }
...
sub makeHTML {
  ...
  if (open(INDX, $WRITELAYER, "$p/index.html")) {
...
';
    ...
    &ErrorBug("makeHTML: Could not open \"$p/index.html\" for writing");
  }
}
