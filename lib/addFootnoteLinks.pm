  ...
  if (-e $commandFile) {
    ...
    while (<CF>) {
      ...
      if ($_ =~ /^(\#.*|\s*)$/) {next;}
        ...
        my $fix = $1;
        if ($fix !~ s/\bLOCATION='(.*?)(?<!\\)'//) {
          &Error("Could not find LOCATION='book.ch.vs' in FIX statement: $_", "Change this FIX statement in CF_addFootnoteLinks.txt. See top of addFootnoteLinks.pm.");
          next;
        }
        my $location = $1;
        if ($fix !~ s/\bAT='(.*?)(?<!\\)'//) {
          &Error("Could not find AT='reference text' in FIX statement: $_", "Change this FIX statement in CF_addFootnoteLinks.txt. See top of addFootnoteLinks.pm.");
          next;
        }
        my $at = $1; $at =~ s/\\'/'/g;
        if ($fix !~ s/\b(REPLACEMENT)='(.*?)(?<!\\)'//) {
          &Error("Could not find REPLACEMENT='exact-replacement' in FIX statement: $_", "Change this FIX statement in CF_addFootnoteLinks.txt. See top of addFootnoteLinks.pm.");
          next;
        }
        ...
        &Error("\"$_\" in command file CF_addFootnoteLinks.txt was not handled.", "Change or remove this entry. See top of addFootnoteLinks.pm.");
      }
    }
    close (CF);
  }
