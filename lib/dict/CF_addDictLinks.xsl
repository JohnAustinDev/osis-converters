 ...
 exclude-result-prefixes="#all">
  ...
  <template match="/">
    ...
    <variable name="link_targets" as="element(oc:target)*">
      <for-each select="
          (descendant::seg[@type='keyword'] | descendant::milestone)
          [@osisID]
          [not(ancestor::div[@subType='x-aggregate'])]
          [not(
            ancestor::div[@annotateType='x-notConversion']
            [
              tokenize(@annotateRef, '\s+') =
              ('CF_addDictLinks', concat('CF_addDictLinks.', $type))
            ]
          )]">
        <oc:target osisRef="{concat($MOD, ':' , replace(@osisID, '\.dup\d+!toc', '!toc'))}"
            ...
            subType="{ancestor::div[@subType][last()]/replace(@subType, '^x-', '')}"
            osisID="{@osisID}"/>
      </for-each>
    </variable>
...
<with-param name="msg">Writing default CF_addDictLinks.xml (type=<value-of select="$type"/>): <value-of select="$output"/> from: <value-of select="base-uri()"/>.</with-param>
    ...
    <addDictLinks version="1.0" xmlns="http://github.com/JohnAustinDev/osis-converters">
      <div notXPATH="{$notXPATH_default}">
        ...
        <for-each-group select="$link_targets"
            group-by="concat('scope=', @scope, ', subType=', @subType)"
            xmlns="http://www.w3.org/1999/XSL/Transform">
          ...
          <element name="div" namespace="http://github.com/JohnAustinDev/osis-converters">
            <if test="matches(current-grouping-key(), 'scope=([^,]+),')">
              <attribute name="context" select="
                  replace(current-grouping-key(), '^scope=([^,]+),.*?$', '$1')"/>
            </if>
            <if test="matches(current-grouping-key(), 'subType=(.+)$')">
              <attribute name="subType" select="
                  replace(current-grouping-key(), '^.*subType=(.+)$', '$1')"/>
            </if>
